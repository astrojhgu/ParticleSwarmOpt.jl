...
function Particle(position::V, velocity::V, fitness::eltype(V))::Particle{V} where {V<:AbstractArray}
    Particle{V}(position, velocity, fitness, missing)
end

zeros_like(v::Array)=zeros(eltype(v), size(v)...)

function Particle(f::Function, position::V)::Particle{V} where {V<:AbstractArray}
...
function swarm_from_ensemble(func::Function, ensemble::AbstractArray{V,1})::Array{Particle{V}, 1} where {V<:AbstractArray}
    p(v)=Particle(func, v)
    map(p, ensemble)
end


function ParticleSwarmMaximizer(func::Function, ensemble::AbstractArray{V,1}, guess::Union{Missing, V}) where {V<:AbstractArray}
...
function move(p::Particle{V}, velocity, func::Function) where {V<:AbstractArray}
...
function sample(psm::ParticleSwarmMaximizer{V}, w, c1, c2, rng::Random.AbstractRNG=Random.GLOBAL_RNG) where {V<:AbstractArray}
    T=eltype(V)
    for p in psm.swarm
...
function converged_dfit(psm::ParticleSwarmMaximizer{V}, p, m)::Bool where {V<:AbstractArray}
    if ismissing(psm.gbest)
        false
    else
...
function converged_dspace(psm::ParticleSwarmMaximizer{V}, p, m)::Bool where {V<:AbstractArray}
    if ismissing(psm.gbest)
        false
    else
...
function converged(psm::ParticleSwarmMaximizer{V}, p, m1, m2)::Bool where {V<:AbstractArray}
